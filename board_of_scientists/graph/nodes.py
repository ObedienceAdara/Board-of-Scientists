...
def node_analyst(state):
    original_text = state["paper_corpus"].full_paper_text
    result = _run_agent(analyst_agent, state)
    if original_text and len(result["paper_corpus"].full_paper_text) < len(original_text):
        result["paper_corpus"].full_paper_text = original_text
    return result
...
def node_quality_gate_failed(state: ResearchState) -> ResearchState:
    """Terminate with an unresolved verdict rather than falsely approving work."""
    runtime = to_runtime_state(state)
    failed_agents = [
        key for key, count in runtime.get("revision_counts", {}).items()
        if count >= 3 and key in runtime.get("evaluations", {})
    ]
    names = ", ".join(failed_agents) or "one or more agents"
    return from_runtime_state({
        **runtime,
        "final_verdict": (
            "UNRESOLVED — QUALITY GATE EXHAUSTED. "
            f"{names} reached the maximum revision budget without a trustworthy approval. "
            "The run must not be represented as COMPLETE."
        ),
    })
...
def node_output(state: ResearchState) -> ResearchState:
    ...
    measured_appendix = json.dumps(measured, indent=2) if measured else "No measured execution results are available for this run."
    sections = [
        ...
        {"title": "Validation Report (Experiment Engineer's analysis — see appendix for raw measurements)", "content": runtime.get("validation_report", "")},
        {"title": "Appendix: Raw Measured Execution Results (ground truth, not LLM-generated)", "content": measured_appendix},
        {"title": "CRO Final Verdict", "content": runtime.get("final_verdict", "")},
        {"title": "Team Communications Log", "content": json.dumps(runtime.get("message_board", []), indent=2)},
    ]
...
__all__ = [
    "node_analyst", "node_cro_read", "node_theorist", "node_architect", "node_cro_plan",
    "node_engineer", "node_reviewer", "node_experiment", "node_writer", "node_cro_verdict",
    "node_quality_gate_failed", "node_eval_analyst", "node_eval_theorist", "node_eval_architect",
    "node_eval_engineer", "node_eval_reviewer", "node_eval_experiment", "node_eval_writer", "node_output",
]
